...
void DlgResizeHelper::OnSize()
{
  if (::IsWindow(m_hParent))
  {
    ...
    for (it=m_ctrls.begin(); it!=m_ctrls.end(); ++it)
	{
      ...
      // resize child window
      ::MoveWindow(it->m_hCtrl, currCtrlSize.left, currCtrlSize.top-2, currCtrlSize.Width(), currCtrlSize.Height(), TRUE);
    }
  }
}
...
UINT DlgResizeHelper::Fix(LPCTSTR a_pszClassName, EHFix a_hFix, EVFix a_vFix)
{
  ...
  for(it = m_ctrls.begin(); it!=m_ctrls.end(); ++it) 
  {
    ::GetClassName(it->m_hCtrl, (LPWSTR)pszCN, sizeof(pszCN));
